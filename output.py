...
def patient_rank(df, uniq, inputmean):
    ...
    for item in uniq:
        ...
        newtemp = temp.ix[temp['chromosome'] != 'Y', :]	
...
mp, bp = np.polyfit(top_patmeans['input_mean'], top_patmeans['cancer_mean'], 1)

plt.clf()
plt.plot(top_patmeans['input_mean'], top_patmeans['cancer_mean'], '.')
plt.plot(top_patmeans['input_mean'], mp*top_patmeans['input_mean'] + bp, '-')
plt.xlabel('input_mean')
plt.ylabel('cancer_mean')
plt.title('Top Patient Correlation')
#plt.legend(loc = 'lower right', scatterpoints = 0)
#plt.text(xytext='axes fraction','R^2= %.2f'%corr, verticalalignment='bottom', horizontalalignment='right')
